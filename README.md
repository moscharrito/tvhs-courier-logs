...
**Every time on screen is in the project's timezone, never the device's** (`web/src/lib/when.ts`). Deadlines in this contract are wall-clock times in America/Chicago: the server computes them there, `pod.ts` prints them there, and the SLA report measures against them there. Until the walkthrough of 14 September the screens did not, because they called `toLocaleTimeString` with no zone, which means whatever zone the browser is set to. A delivery made at 1:08 PM read "2:08 PM" on an Eastern laptop while the proof-of-delivery PDF linked from the same page said 1:08 PM. Nothing was ever mismeasured, since on time is decided server-side from instants; only the label was wrong, and a label that disagrees with the paper is how a billing dispute starts. In San Antonio the two answers coincide, which is why it survived four phases. `clockFor`, `stampFor` and `momentFor` take the zone and cache a formatter per zone; `todayIn` does the same for the service-date default, where the device's idea of today is the wrong contract day near midnight. `safeZone` falls back to the device rather than throwing, because a project timezone is a string somebody typed into the settings screen. The account screens (audit log, devices, a user's sessions) deliberately stay in the reader's own zone through `fmtWhen`: those answer a personal question and two of them are not scoped to a project at all. `web/src/lib/when.test.ts` pins the split, and asserts in four named zones so it fails wherever it is run if the zone goes missing again.
...
The dispatch number is `dispatch.phone` in the project settings, set on the operating settings card. There is no default, and the button is hidden until someone sets one: a courier standing at a door with a problem would dial whatever is there, so a placeholder is worse than nothing. It had no default and no way to set it either until the walkthrough of 14 September found the run screen saying "No dispatch number is set for this project yet" to the one person who could not do anything about it. It is on the settings table rather than buried, marked in red while it is empty, and the courier's message now names who fixes it.
...
**The PIN belongs to the device** (`devices.pin`), not to the person. It lived on `users.pin` until ticket 5.8, the same column the legacy TVHS route quick-login is keyed on, and a route PIN is accepted from any device. So enrolling a phone quietly rewrote a credential that worked from anywhere, which is precisely the property the device binding exists to remove: a driver with a route who set their phone to 4821 could then sign in as themselves from any browser by typing `northbound` and 4821. Three smaller consequences came from the same column. A second phone changed the first one's PIN. Resetting a route PIN changed what an enrolled phone expected. And `PUT /api/users/:username/pin` set what somebody's phone would accept, so an administrator resetting a courier's PIN handed themselves the four digits that unlock that courier's handset.

Now enrolment writes only the device row and revoking a phone nulls it, which is what the devices screen has always claimed it does; `users.pin` is the route PIN and nothing else, and the admin endpoint refuses for a user with no route rather than reporting a reset that reset nothing. Migration `0026_device_pin` copies each live device's PIN across so phones enrolled before it keep working, leaves revoked rows null, and clears `users.pin` for anyone with no route, where it could authenticate nothing.
...
**Ticket 4.9 then took the import itself.** It wrote an order, a package and a custody event per row, each awaited separately: nine hundred statements for three hundred rows. They are now two batches, the order inserts with `RETURNING id` and then the children built from the ids that come back. The import halved, from 364 ms to 192 ms, and so did the wait it imposes on a courier's manifest read beside it. The relationship did not change and will not: the read takes as long as the import, because it is one process on one connection, and what is left of the import's time is the parse and the duplicate check rather than the writes.

Batching also made the import **atomic**, which it was not. A failure part way through used to leave the orders it had already written behind a list row claiming a count that was no longer true. Two batches can still produce one partial state, orders written and their packages and custody events not, so that path takes the orders back out and fails: an order with no chain of custody is worse than no order.
...
`web/` is a Vite + React app served by the server at `/` from `web/dist` (any non-API, non-file GET falls back to `index.html` for the client router). It is branded TAG. Sign-in asks which project first (`GET /api/login/projects`, public, names only), then shows that project's couriers (`GET /api/drivers/list?project=<code>`); staff sign in with username and password from any step. **What picking a courier does depends on whether the project uses routes.** Routes are a TVHS idea, two vans with one driver each, and the route is both the label and the identifier PIN sign-in is keyed on; a courier with one gets PIN entry or first-time setup as before. A courier without one gets a password box, and then the run screen offers to enrol the phone (ticket 5.4) so every sign-in after that is a PIN bound to that phone. They are deliberately not offered a route-style PIN, which works from any device: minting a second four-digit any-device route to PHI would undo what the device-bound one is for. Requiring a route was why every UH courier was missing from the roster and told, wrongly, that nobody was set up; membership decides it now, and the username travels with the name because the picker is what signs them in. Platform admins are excluded from the roster even when somebody gives them a courier membership. After sign-in everyone lands on a project picker, and the platform screens for admins (Users, Audit log) and everyone (My devices). The original TVHS courier log app is served under `/legacy` and mounted inside the shell without an iframe: the shell injects its markup, loads its script once, and re-enters it through its global `checkSession()`; sign-out is routed through the shell. Drivers pick their project like everyone else. A project page carries its sites, its contract pricing and its operating settings; the settings card shows each value beside the contract default, marks anything a person changed, and restates the current rules as real due times, but only an admin or ops manager sees an Edit button. The last card on the page says what is genuinely not built yet, which is address lookup and nothing else; it claimed the dispatch board and the courier app were still coming for two phases after they shipped, and a page that describes the product as unfinished when it is not is a page nobody trusts the rest of. During development run `npm run dev -w server` and `npm run dev -w web` side by side.
