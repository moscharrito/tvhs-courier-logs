...
| 0.7 | User management | 1.5 | Admin screen and API: create user, set role per project, reset password, set or clear PIN, deactivate. Env-var seeding reduced to a one-time bootstrap admin. TVHS driver rows migrate to memberships with role courier and a settings blob carrying their route |
...
| 1.5 | Daily list import | 2.5 | Upload xlsx or csv per site. Column mapping saved per site after first use. Preview with validation errors. Creates daily_list and orders with packages. Duplicate detection within the day. Geocode failures land in a review queue. Tested with the redacted one-day Discharge Pharmacy sample |
