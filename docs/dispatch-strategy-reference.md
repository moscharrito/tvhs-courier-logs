...
| 5. Grouping into dense loops | Runs and stops; dispatch board with nearest-neighbor sequencing from the origin site; full optimization deferred past go-live | 2.1, 2.2 | 2.1 and 2.2 done. Nearest-neighbour is written and tested but refuses until ticket 1.4 supplies coordinates; sequencing by deadline works today. Full optimization still deferred |
| 6. County-wide and street-level live view | Dispatch board with courier lanes and minutes-to-due; live status feed on each courier event; map link per stop | 2.2, 2.8 | Board done: lanes, minutes-to-due badges, courier presence, 15-second polling. Still list-first; the map layer needs 1.4 and remains an addition |
