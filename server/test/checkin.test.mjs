...
describe('driver check-in', () => {
    it('reports not checked in before the first check-in', async () => {
        ...
        const res = await a.get('/api/projects/tvhs/tvhs/checkin?date=2026-01-05');
    ...
    it('checks in for a client-supplied date and is idempotent', async () => {
        const a = await srv.login('north');
        const first = await a.post('/api/projects/tvhs/tvhs/checkin').send({ date: '2026-01-05' });
        ...
        const second = await a.post('/api/projects/tvhs/tvhs/checkin').send({ date: '2026-01-05' });
    ...
    it('falls back to the server date when the client date is invalid', async () => {
        ...
        const res = await a.post('/api/projects/tvhs/tvhs/checkin').send({ date: 'not-a-date' });
    ...
    it('returns own history in date order and honours the range filters', async () => {
        const a = await srv.login('north');
        await a.post('/api/projects/tvhs/tvhs/checkin').send({ date: '2026-01-07' });
        await a.post('/api/projects/tvhs/tvhs/checkin').send({ date: '2026-01-06' });

        const all = await a.get('/api/projects/tvhs/tvhs/checkins/history?startDate=2026-01-01&endDate=2026-01-31');
        expect(all.body.map(r => r.date)).toEqual(['2026-01-05', '2026-01-06', '2026-01-07']);

        const narrowed = await a.get('/api/projects/tvhs/tvhs/checkins/history?startDate=2026-01-06');
...
describe('admin check-in views', () => {
    it('roster lists every driver with their status for the date', async () => {
        ...
        const res = await admin.get('/api/projects/tvhs/tvhs/admin/checkins?date=2026-01-05');
        expect(res.status).toBe(200);
    ...
    it('history filters by driver, route and date range', async () => {
        ...
        const all = await admin.get('/api/projects/tvhs/tvhs/admin/checkins/history?startDate=2026-01-01&endDate=2026-01-31');
        expect(all.body).toHaveLength(3);
        expect(all.body[0]).toMatchObject({ username: 'north.driver', driver_name: 'Bereket Nigusse', driver_route: 'northbound' });

        const byRoute = await admin.get('/api/projects/tvhs/tvhs/admin/checkins/history?route=southbound&startDate=2026-01-01&endDate=2026-01-31');
        expect(byRoute.body).toHaveLength(0);

        const byDriver = await admin.get('/api/projects/tvhs/tvhs/admin/checkins/history?driver=north.driver&startDate=2026-01-06&endDate=2026-01-06');
        expect(byDriver.body.map(r => r.date)).toEqual(['2026-01-06']);

        const allKeyword = await admin.get('/api/projects/tvhs/tvhs/admin/checkins/history?driver=all&route=all&startDate=2026-01-01&endDate=2026-01-31');
