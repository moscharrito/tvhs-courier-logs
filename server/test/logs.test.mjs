...
describe('route definitions', () => {
    it('exposes the two fixed routes with their legs and default miles', async () => {
        ...
        const res = await a.get('/api/projects/tvhs/tvhs/routes');
...
describe('driver daily logs', () => {
    it('rejects a save without date or legs', async () => {
        const a = await srv.login('north');
        expect((await a.post('/api/projects/tvhs/tvhs/logs').send({ legs: [] })).status).toBe(400);
        expect((await a.post('/api/projects/tvhs/tvhs/logs').send({ date: '2026-01-05' })).status).toBe(400);
    });

    it('saves standard and extra legs, coercing numbers and storing extra leg labels', async () => {
        const a = await srv.login('north');
        const save = await a.post('/api/projects/tvhs/tvhs/logs').send({ date: '2026-01-05', legs: northDay() });
        expect(save.status).toBe(200);
        expect(save.body).toEqual({ ok: true });

        const rows = (await a.get('/api/projects/tvhs/tvhs/logs?startDate=2026-01-05&endDate=2026-01-05')).body;
    ...
    it('coerces malformed numeric input to zero and truncates long labels to 60 chars', async () => {
        ...
        expect((await a.post('/api/projects/tvhs/tvhs/logs').send({ date: '2026-01-05', legs })).status).toBe(200);
        const rows = (await a.get('/api/projects/tvhs/tvhs/logs?startDate=2026-01-05&endDate=2026-01-05')).body;
    ...
    it('re-saving with fewer legs upserts the rest and deletes the dropped extra legs', async () => {
        ...
        shorter[0].miles = 81;
        expect((await a.post('/api/projects/tvhs/tvhs/logs').send({ date: '2026-01-05', legs: shorter })).status).toBe(200);
    ...
    it('a driver only ever sees their own logs even when asking for another username', async () => {
        ...
        const rows = (await south.get('/api/projects/tvhs/tvhs/logs?username=north.driver&startDate=2026-01-05&endDate=2026-01-05')).body;
    ...
    it('DELETE clears a whole day for the driver', async () => {
        const a = await srv.login('south');
        expect((await a.delete('/api/projects/tvhs/tvhs/logs').send({})).status).toBe(400);
        expect((await a.delete('/api/projects/tvhs/tvhs/logs').send({ date: '2026-01-05' })).body).toEqual({ ok: true });
        expect((await a.get('/api/projects/tvhs/tvhs/logs?startDate=2026-01-05&endDate=2026-01-05')).body).toHaveLength(0);
    });
});
...
describe('admin log views', () => {
    beforeAll(async () => {
        ...
        await north.post('/api/projects/tvhs/tvhs/logs').send({ date: '2026-01-06', legs: northDay() });
        const south = await srv.login('south');
    ...
    it('admin can read any driver by username', async () => {
        ...
        const rows = (await admin.get('/api/projects/tvhs/tvhs/logs?username=south.driver&startDate=2026-01-06&endDate=2026-01-06')).body;
        expect(rows).toHaveLength(2);
    ...
    it('lists all logs joined to driver name and route, ordered by date, driver, leg', async () => {
        const admin = await srv.login('admin');
        const rows = (await admin.get('/api/projects/tvhs/tvhs/admin/logs?startDate=2026-01-01&endDate=2026-01-31')).body;
    ...
    it('filters by driver, by route and by date range', async () => {
        const admin = await srv.login('admin');
        expect((await admin.get('/api/projects/tvhs/tvhs/admin/logs?route=southbound')).body).toHaveLength(2);
        expect((await admin.get('/api/projects/tvhs/tvhs/admin/logs?driver=north.driver&startDate=2026-01-06')).body).toHaveLength(7);
        expect((await admin.get('/api/projects/tvhs/tvhs/admin/logs?driver=all&route=all&endDate=2026-01-05')).body).toHaveLength(6);
    });

    it('stats count drivers, distinct log days, miles and totes', async () => {
        const admin = await srv.login('admin');
        const s = (await admin.get('/api/projects/tvhs/tvhs/admin/stats')).body;
    ...
    it('driver list for admin filters', async () => {
        const admin = await srv.login('admin');
        const d = (await admin.get('/api/projects/tvhs/tvhs/admin/drivers')).body;
        expect(d).toEqual([
