...
const MIGRATION_TAGS = ['0000_baseline', '0001_projects', '0002_sessions', '0003_users', '0004_audit', '0005_uh_project', '0006_sites'];
    ...
    it('creates the legacy tables exactly (plus project_id), the core tables, and records both migrations', async () => {
        ...
        try {
            ...
            expect(idx.rows.map((r) => r.name)).toEqual([
                ...
                'sessions_user_id_idx', 'sites_project_code_unique', 'sites_project_id_idx',
            ]);
